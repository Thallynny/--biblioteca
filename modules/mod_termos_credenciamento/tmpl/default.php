...
foreach ($dados['listaTermoCredenciamento']['return'] as  $group) {
	...
	if (!empty($horarioAno2[2])) {
		...
		$dados['listaTermoCredenciamento']['return'][$i1]['ordemDescricao'] = getOrdenacao($group['termos'][0]['descricaoArquivo']);
	}
	$i1++;
}
...
$listaAuxiliar = $dados['listaTermoCredenciamento']['return'];
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if (!empty($listaPdf['exibe'])) :
                                $texto .= "<a title='Link para: ". $listaPdf['descricaoArquivo'] ."' href='#conteudo' onclick=\"exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '".$listaPdf['codigo']."', 'SV1', '".urlencode("CONVÊNIOS E ACORDOS/Termos de Credenciamento/" . $horarioAno2[2] . "/" . $listDados['instituicao'] . "/" . $listaPdf['descricaoArquivo'])."')\" >";
                                $texto .= $listaPdf['descricaoArquivo'] . "</a>";
                                ?>
                            <a  title="Link para: <?php echo $listaPdf['descricaoArquivo']; ?>" href="#conteudo" onclick="exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $listaPdf['codigo']; ?>', 'SV1', '<?php echo urlencode('CONVÊNIOS E ACORDOS/Termos de Credenciamento/' . $horarioAno2[2] . '/' . $listDados['instituicao'] . '/' . $listaPdf['descricaoArquivo']);  ?>')">
                        ...
                        <td>
                            <?php foreach ($listDados['aditivos'] as $listAditivo) :
                                $texto .= "<br>";
                                $texto .= "<a  title='Link para: ". $listAditivo['descricaoArquivo'] ."' href='#conteudo' onclick=\"exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '".$listAditivo['codigo']."', 'SV12', '".urlencode("CONVÊNIOS E ACORDOS/Termos de Credenciamento/" . $horarioAno2[2] . "/" . $listDados['instituicao'] . "/" . $listAditivo['descricaoArquivo'])."')\">";
                                $texto .= $listAditivo['descricaoArquivo'] . "</a>";
                                ?>
                            <br><a  title="Link para: <?php echo $listAditivo['descricaoArquivo']; ?>" href="#conteudo" onclick="exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $listAditivo['codigo']; ?>', 'SV12', '<?php echo urlencode('CONVÊNIOS E ACORDOS/Termos de Credenciamento/' . $horarioAno2[2] . '/' . $listDados['instituicao'] . '/' . $listAditivo['descricaoArquivo']);  ?>')"><?php echo $listAditivo['descricaoArquivo']; ?></a>
                            <?php endforeach;
                        $texto .= "</td>"
                        ?>
                        </td>

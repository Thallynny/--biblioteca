...
$arguments = array('getDemonstrativoGestaoOrcamentaria' => array('arg0' => $secao,'arg1' => $idCategoria));
					...
					if(!empty($listb) && !empty($listb->descricao)){
						...
						echo " 
							...
							<div style='display:none;'>
								<input type='text' id='p_secao' value='".$secao."' />
							</div>";
						$anoDesc = "";	
						foreach ($listb as $listBano){
							if(is_array($listBano)){
								foreach ($listBano as $anoTemp) {
									...
									if(is_object($anoTemp->meses)){
										...
										if($listBanoMes->descricaoMes != $mesRepetido){
											...
											else if($listBanoMes->visaoRegistrada == 2){
												echo "<li><a href='#conteudo' role='button' onclick=\"exibirArquivo('../../gestao-orcamentaria/resultado-pdf', '".$listBanoMes->id."', 'SV18', '".urlencode("GESTÃO ORÇAMENTÁRIA/GESTÃO - DEMONSTRATIVOS E RELATÓRIOS/DEMONSTRATIVOS/".getDescricaoSecao($list->demonstrativo->descricaoSecao)."/".retiraBarras($listb->descricao)."/".$anoTemp->ano)."')\">".$listBanoMes->descricaoMes."</a></li>";
											}
											else if($listBanoMes->visaoRegistrada == 3){
								...
								$conjBotoes = "";
								$mesRepetido = "";
								if(is_object($listBano->meses)){
									$listBanoMes = $listBano->meses;
									if($listBanoMes->descricaoMes != $mesRepetido){
										$mesRepetido = $listBanoMes->descricaoMes;
								...
								}else if(is_array($listBano->meses)){

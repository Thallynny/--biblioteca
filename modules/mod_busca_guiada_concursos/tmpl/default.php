...
$arg0 = (!empty($_GET['arg0'])) ? $_GET['arg0'] : "concursos-e-selecoes";
$origem = (!empty($_GET['origem'])) ? $_GET['origem'] : "0";
...
$titleInput = "Digite um texto sobre o assunto que deseja encontrar em Concursos";

//Rotina para navegar adiante nas telas
$origens = "";
$listaOrigem = explode("/", $origem);
if (!empty($listaOrigem[count($listaOrigem) - 2])) {
	$origens = $listaOrigem[count($listaOrigem) - 2]; //pega a penultima posição para preencher a opção do bot�o voltar
...
if ($nivel == "resultado") {	?>
	...
	<div class="row">
		<div class="col-12 consultacontainer">
			<div class="box mt-2">
				Não foi possível carregar a consulta geral, <a href="#" onclick="javascript:resultadoBuscaGuiada('concursos-e-selecoes','1','0','0','conciliacao');" title=" Tente Novamente">Tente novamente</a>.
						...
						foreach ($retorno->categoria as $botoes) {
							if (is_object($botoes)) {
								if (!empty($botoes->link) && $botoes->link != "") {
									$link = str_replace("concursos-e-selecoes/", "", $botoes->link);
					?>
									<li class="left">
										<a class="box noborder" style="<?php echo tamanhoFontBotao($botoes->title); ?>" href="<?php echo $link; ?>" <?php echo strpos($link, 'index.php/') ? '' : 'target="_blank"'; ?> title="Link para <?php echo $botoes->title; ?>">
											<?php echo getTitle($botoes->title); ?>
										</a>
									</li>
								<?php } else { ?>
									<li class="left">
										<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->id; ?>','<?php echo "concursos-e-selecoes/" . $botoes->id; ?>','<?php echo $nivel + 1; ?>','','<?php echo $botoes->title; ?>', 'avancar')" ;>
											<?php echo getTitle($botoes->title); ?>
										</button>
									</li>
					<?php }
...
if ($nivel >= 1 and $nivel  <= 100) {
	$arg = $arg0;
	$subNivel = $nivel;
	$origemAberta = "concursos-e-selecoes";
	$paginaAberta = (!empty($_GET['pagina'])) ? $_GET['pagina'] : 0;
	$limiteAberta = (!empty($_GET['limite'])) ? $_GET['limite'] : 5;
	$termoBuscaAberta = (!empty($_GET['termobusca'])) ? $_GET['termobusca'] : "";
								...
								if (is_object($botoes)) {
									if ($botoes->exibe) {
										if (!empty($botoes->sv) && $botoes->sv != "") { ?>
											<li class="left">
												<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" style="<?php echo tamanhoFontBotao($botoes->title); ?>" onClick="exibirArquivo('../index.php/concursos-e-selecoes/resultado-pdf', '<?php echo $botoes->id; ?>', '<?php echo $botoes->sv; ?>', '<?php echo $tituloNiveis; ?>')">
													<?php echo getTitle($botoes->title); ?>
												</button>
											</li>
										<?php
										...
										<?php } else { ?>
											<li class="left">
												<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" style="<?php echo tamanhoFontBotao($botoes->title); ?>" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->path; ?>','<?php echo $origem . "/" . $botoes->path; ?>','<?php echo $nivel + 1; ?>','<?php echo $botoes->params; ?>','<?php echo $tituloNiveis . "/" . $botoes->title; ?>','avancar')" ;>
													<?php echo getTitle($botoes->title); ?>
												</button>
											</li>
					<?php }

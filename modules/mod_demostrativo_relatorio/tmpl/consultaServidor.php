...
$arguments = array('getDemonstrativoGestaoOrcamentaria' => array('arg0' => $secao, 'arg1' => '0'));
		...
		foreach($list->demonstrativo as $lista){
			if(is_array($lista) ){
				...
				foreach($lista as $listb){
					if(!empty($listb) && !empty($listb->descricao)){
						$contadorDivisao++;
						echo "	<div class='row'>
									...
									if(is_object($anoTemp->meses)){
										...
										if($listBanoMes->descricaoMes != $mesRepetido){
											...
											if($listBanoMes->visaoRegistrada == 1){
												echo "<li><a title='Abrir ".$listBanoMes->descricaoMes."' href='#conteudo' role='button' onClick=\"javascript:exibirBotaoRelatorio('".$contadorDivisao.$idAnoMes."', this)\">".$listBanoMes->descricaoMes."</a></li>";
											}
											if($listBanoMes->visaoRegistrada == 2){
												echo "<li><a  title='Abrir ".$listBanoMes->descricaoMes."' href='#conteudo' role='button' onclick=\"exibirArquivo('../gestao-orcamentaria/resultado-pdf', '".$listBanoMes->id."', 'SV18', '".urlencode("GESTÃO ORÇAMENTÁRIA/GESTÃO - DEMONSTRATIVOS E RELATÓRIOS/DEMONSTRATIVOS/".getDescricaoSecao($list->demonstrativo->descricaoSecao)."/".retiraBarras($listb->descricao)."/".$anoTemp->ano)."')\">".$listBanoMes->descricaoMes."</a></li>";
											}
											if($listBanoMes->visaoRegistrada == 3){
												echo "<li><a  title='Abrir ".$listBanoMes->descricaoMes."' href='#conteudo' role='button' onClick=\"javascript:exibirModalDemostrativoRelatorio(".$listBanoMes->id.") \">".$listBanoMes->descricaoMes."</a></li>";
											}
											$botao = "";
											...
											foreach($listBanoMes as $anexo){
												if(is_object($anexo)){
													$id = $anexo->id;
													$botao .= "<li><a title='Link para: ".$anexo->descricao."'  href='#conteudo' role='button' class='box' onClick=\"exibirArquivo('../gestao-orcamentaria/resultado-pdf', '".$id."', 'SV17', '".urlencode("GESTÃO ORÇAMENTÁRIA/GESTÃO - DEMONSTRATIVOS E RELATÓRIOS/DEMONSTRATIVOS/".getDescricaoSecao($list->demonstrativo->descricaoSecao)."/".$listb->descricao."/".$anexo->descricao."/".$anoTemp->ano)."')\" >".$anexo->descricao."</a></li>";
												}
											}

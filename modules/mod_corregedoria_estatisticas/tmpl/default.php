		...
		?>
            <div class="row botoes2 boxConteudoMes" id="<?=$dado['ano']."-".$mes['descricaoMes']?>"> 
                <ol>
					...
					if(is_array($anexo)){?>
						<li>
                            <a href='#conteudo' onclick="exibirArquivo('index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $anexo['id']; ?>', 'SV35', '<?php echo urlencode('CORREGEDORIA/ESTATÍSTICA/'.$dado['ano'].'/'.$mes['descricaoMes'].'/'.$anexo['descricao'])  ?>')">
								<?=$anexo['descricao'];  ?>
							</a>                             
						</li>
					<?php }else{ ?>
						<li>
                            <a href='#conteudo' onclick="exibirArquivo('index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $anexo['id']; ?>', 'SV35', '<?php echo urlencode('CORREGEDORIA/ESTATÍSTICA/'.$mes['anexos']['ano'].'/'.$mes['anexos']['descricaoMes'].'/'.$mes['anexos']['descricao'])  ?>')">

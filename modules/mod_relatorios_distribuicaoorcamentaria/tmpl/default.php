				...
				if ($anoTermo != $dados['ano']) {
					...
					<div class="row report">
						<ul>
							...
							<li class="arrow-down">
								<a title="Expandir o ano de <?= $dados['ano']; ?>" href="#report"><img src="templates/portalTRF5/images/arrow_down_2.svg"></a>
							</li>
						</ul>
					</div>
						...
						<ul>
							<?php foreach ($lista['distribuicaoOrcamentaria'] as $arquivo) : ?>
								<?php if ($dados['ano'] == $arquivo['ano']) : 	?>
									<li class="inline">
										<div class="box" role="button"><a title="Link para: <?= $arquivo['descricao']; ?>" href="#conteudo" onclick="exibirArquivo('../gestao-orcamentaria/resultado-pdf', '<?php echo $arquivo['id']; ?>', 'SV16', '<?php echo urlencode('GESTÃO ORÇAMENTÁRIA/DISTRIBUIÇÃO ORÇAMENTÁRIA/' . $dados['ano'] . '/' . retiraBarras($arquivo['descricao']));  ?>')">
											<?= $arquivo['descricao']; ?></a>
										</div>
									</li>
								<?php endif; ?>

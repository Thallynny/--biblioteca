...
<div class="container demonstrativo bg_azul_fundo">
	<div class="row">
		<div class="col-md-6 aba selecionado" data-aba="1">
			<div><a class="textoSemSublinhado" href="#container" title="Posição Patrimonial">Posição Patrimonial</a></div>
		</div>
		<div class="col-md-6 aba" data-aba="2">
			<div><a class="textoSemSublinhado" href="#container" title="Termos de Doação">Termos de Doação</a></div>
		</div>
	</div>
</div>
				...
				if ($ano != $posicao['anoPublicacao']) {
					...
					<div class="row report">
						<ul>
							...
							<?php 
							foreach ($list['posicaoPatrimonial'] as $mes){
								if ($ano == $mes['ano']){
									echo "<li><a role='img' href='#conteudo' onclick=\"exibirArquivo('../index.php/gestao-orcamentaria/resultado-pdf', '".$mes['id']."', 'SV8', '".urlencode("PORTAL DA TRANSPARÊNCIA/GESTÃO PATRIMONIAL/ADMINISTRAÇÃO PATRIMONIAL DE BENS MÓVEIS/".$mes['id'])."')\" title='Expandir o mês: ". $mes['mes'] ."'>
										".$mes['mes']."
										</a></li>";
								}
							} ?>
				...
				if ($anoTermo != $termo['ano']) :
					...
					<div class="row report">
						<ul>
							...
							<li class="arrow-down"><a href="#conteudo" role="button" title="Expandir ano de <?= $termo['ano'] ?>"><img src="templates/portalTRF5/images/arrow_down_2.svg" alt="Abrir conteúdo"></a></li>
						</ul>
					</div>
					<div class="row botoes">
						<ul>
							<?php						
							array_sort_by2($list['termosDoacao'], 'numeroTermo', $order = SORT_DESC);
							foreach ($list['termosDoacao'] as $arquivo): ?>
								<?php if ($termo['ano'] == $arquivo['ano']) : ?>
									<li class="inline">
										<a href="#botoes" role="button" class="box" onclick="exibirArquivo('../gestao-orcamentaria/resultado-pdf', '<?= $arquivo['id'] ;?>', 'SV9', '<?php echo urlencode($tituloNiveis.'/'.$termo['ano'].'/'.retiraBarras($arquivo['descricao'])); ?>')" title="Link para: <?= $arquivo['descricao']; ?>">
											<?= $arquivo['descricao']; ?>
										</a>
									</li>
								<?php endif; ?>

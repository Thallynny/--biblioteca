...
$titleInput = "Digite um texto sobre o assunto que deseja encontrar no Portal da Transparência";
...
if ($nivel == "resultado") {	?>
	...
	<div class="row">
		<div class="col-12 consultacontainer">
			<div class="box mt-2">
				Não foi possível carregar a consulta geral, <a href="#" onclick="javascript:resultadoBuscaGuiada('portal-da-transparencia','1','0','avancar')" title="Tente Novamente">Tente novamente</a>.
						...
						foreach ($retorno->categoria as $botoes) {
							if (is_object($botoes)) {
								if (!empty($botoes->link) && $botoes->link != "") {
									$link = str_replace("portal-da-transparencia/", "", $botoes->link);
					?>
									<li class="left">
								...
								<?php } else { ?>
									<li class="left">
										<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->id; ?>','<?php echo "portal-da-transparencia/" . $botoes->id; ?>','<?php echo $nivel + 1; ?>','','<?php echo $botoes->title; ?>', 'avancar')" ;>
											<?php echo getTitle($botoes->title); ?>
										</button>
									</li>
					<?php }
...
if ($nivel >= 1 and $nivel  <= 100) {
	$arg = $arg0;
	$subNivel = $nivel;
	$origemAberta = "portal-da-transparencia";
	$paginaAberta = (!empty($_GET['pagina'])) ? $_GET['pagina'] : 0;
	$limiteAberta = (!empty($_GET['limite'])) ? $_GET['limite'] : 5;
	$termoBuscaAberta = (!empty($_GET['termobusca'])) ? $_GET['termobusca'] : "";

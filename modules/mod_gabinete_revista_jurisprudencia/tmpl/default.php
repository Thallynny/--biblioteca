...
foreach($list as $group){
	...
	if(!empty($horarioAno2[2])){
		...
		$data = $horarioAno2[2].$horarioAno2[1].$horarioAno2[0];
		$list[$i]['dataOrdem'] = $data;
		$i++;
	}
}
...
if($ordernar){
	...
	array_sort_by($list, 'dataOrdem', $order = SORT_DESC);
}
						...
						<div class="row">
							...
							foreach($list as $listDados){
								...
								if($horarioAno[2] == $anoRevista[2]){
									$ano = $anoRevista[2]; ?>
									<div class="col-3">
										<?php echo "<a href='#conteudo' onclick=\"exibirArquivo('../index.php/gestao-orcamentaria/resultado-pdf', '".$listDados['id']."', 'SV31', '".urlencode("GABINETE REVISTA/REVISTAS DE JURISPRUDÊNCIA/".$ano."/".$listDados['descricao'])."')\">
												<img role='img' style='padding: .25rem; background-color: #fff; border: 1px solid #ddd; border-radius: .25rem; transition: all .2s ease-in-out;  max-width: 122px; height: 170px'

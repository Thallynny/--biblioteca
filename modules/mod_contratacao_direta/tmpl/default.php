	...
	<div class="col-12">
		...
		<div class="row">
			...
			if($ano != $list['ano']):
				...
				?>
				<ul>
				<li onclick="exibirArquivo('../gestao-orcamentaria/resultado-pdf', '<?php echo $list['id'] ;?>', 'SV5', '<?php echo urlencode('LICITAÇÕES E CONTRATOS/Contratações Diretas/Relatórios Mensais das Contratações Diretas 2008 à 2013/'.$list['ano'])?>')"><?php echo $list['ano'];?></li>
				<?php foreach($dados['listaContratacaoDireta']['return'] as $listDados):
					if($list['ano'] == $listDados['ano']):?>
						<a  href="#conteudo" onclick="exibirArquivo('../gestao-orcamentaria/resultado-pdf', '<?php echo $listDados['id'] ;?>', 'SV5', '<?php echo urlencode('LICITAÇÕES E CONTRATOS/Contratações Diretas/Relatórios Mensais das Contratações Diretas 2008 à 2013/'.$list['ano'].'/'.$listDados['mes'])?>')"><?php echo $listDados['mes'];?> </a> 
					<?php endif;
				endforeach;?>
				</ul>			
			<?php	endif; ?>

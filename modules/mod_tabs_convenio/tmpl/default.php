...
if($ordernar){
	array_sort_by($dados['listaConvOrgao']['return'], 'descricaoInstituicao', $order = SORT_DESC);
}
...
if($ordernar){
	array_sort_by($dados['listaConvInstituicao']['return'], 'descricaoInstituicao', $order = SORT_DESC);
}
...
<div class="container demonstrativo bg_azul_fundo">
    <div class="row">
        <div class="col-md-4 aba selecionado" data-aba="1">
			<div><a title="Abrir <?= $dados['dados']['titulo_tab_1'] ?>" class="textoSemSublinhado" href="#container"><?= $dados['dados']['titulo_tab_1'] ?></a></div>
        </div>
        <div class="col-md-4 aba" data-aba="2">
            <div><a  title="Abrir <?= $dados['dados']['titulo_tab_2'] ?>" class="textoSemSublinhado" href="#container"><?= $dados['dados']['titulo_tab_2'] ?></a></div>
        </div>
		<div class="col-md-4 aba" data-aba="3">
            <div><a  title="Abrir <?= $dados['dados']['titulo_tab_3'] ?>" class="textoSemSublinhado" href="#container"><?= $dados['dados']['titulo_tab_3'] ?></a></div>
        </div>
    </div>
    <div class="row conteudo selecionado" data-aba-id="1">
		...
		for($i=count($dados['listaConvOrgao']['return'])-1; $i>=0; $i--){
			...
			$horarioAno = $list['descricaoInstituicao'];
			if($ano != $horarioAno):
				$ano = $horarioAno;
				?>
			<div class="row report">
                <ul>
                    ...
                    <li class="arrow-down"><a title="Expandir ano de <?php echo $horarioAno;?>" href="#conteudo" role="button"><img src="templates/portalTRF5/images/arrow_down_2.svg" alt="Abrir conteúdo"></a></li>
                </ul>
            </div>
            <div class="row botoes w100">
                <a href="#conteudo" title="Baixar em PDF" role="button" class="download" onClick="javascript:baixarPDF(<?= $ano; ?>, 'pdf');">
			...
			foreach($dados['listaConvOrgao']['return'] as $listDados):
				...
				$horarioAno2 = $listDados['descricaoInstituicao'];
				...
				<tr>
                    <td>
						...
						?>
							...
							$texto .= "<a  title='Link para: ". $listaPdf['descricao'] ."' href='#conteudo' onclick=\"exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '".$listaPdf['id']."', 'SV10', '".urlencode("CONVÊNIOS E ACORDOS/Convênios com Orgãos Instituições E Sistemas/".$dados['dados']['titulo_tab_1']."/".retiraBarras($listDados['descricaoInstituicao'])."/".retiraBarras($listaPdf['descricao']))."')\" >";
							$texto .= $listaPdf['descricao']."</a>";
							?>
							<a title="Link para: <?php echo $listaPdf['descricao'];?>" href='#conteudo' onclick="exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $listaPdf['id']; ?>', 'SV10', '<?php echo urlencode('CONVÊNIOS E ACORDOS/Convênios com Orgãos Instituições E Sistemas/'.$dados['dados']['titulo_tab_1'].'/'.retiraBarras($listDados['descricaoInstituicao']).'/'.retiraBarras($listaPdf['descricao'])); ?>')" >
					...
					<!-- <td> -->
					<?php 
						//echo $listDados['descricaoInstituicao']; 
					...
					<td>
						<?php
						array_sort_by($listDados['anexos'], 'descricao', $order = SORT_DESC);
						foreach($listDados['anexos'] as $listaPdf):
						$texto .= "<br>";
						?>
							<br>
							<?php if(!empty($listaPdf['exibe'])):	
								$texto .= "<a title='Link para: ". $listaPdf['descricao'] ."' href='#conteudo' onclick=\"exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '".$listaPdf['id']."', 'SV11', '".urlencode("CONVÊNIOS E ACORDOS/Convênios com Orgãos Instituições E Sistemas/".$dados['dados']['titulo_tab_1']."/".retiraBarras($listDados['descricaoInstituicao'])."/".retiraBarras($listaPdf['descricao']))."')\">";
								$texto .= $listaPdf['descricao']."</a>";
							?>
							<a  title="Link para: <?php echo $listaPdf['descricao'];?>" href='#conteudo' onclick="exibirArquivo('/index.php/gestao-orcamentaria/resultado-pdf', '<?php echo $listaPdf['id']; ?>', 'SV11', '<?php echo urlencode('CONVÊNIOS E ACORDOS/Convênios com Orgãos Instituições E Sistemas/'.$dados['dados']['titulo_tab_1'].'/'.retiraBarras($listDados['descricaoInstituicao']).'/'.retiraBarras($listaPdf['descricao'])); ?>')">
									<?php echo $listaPdf['descricao'];?>
							</a>	
							<?php  endif;	?>
						<?php endforeach;
...
<?php
		...
		for($i=count($dados['listaConvInstituicao']['return'])-1; $i>=0; $i--){
			...
			<div class="row report">
                <ul>
                    <li class="titulo"><?php echo $horarioAno;?></li>
                    <li class="arrow-down"><a href="#conteudo" role="button"  title="Expandir ano de <?php echo $horarioAno;?>" ><img src="templates/portalTRF5/images/arrow_down_2.svg" alt="Abrir conteúdo"></a></li>
                </ul>
            </div>
            <div class="row botoes w100">
                <a href="#conteudo" title="Baixar em PDF"  role="button" class="download" onClick="javascript:baixarPDF('2_<?= $ano; ?>', 'pdf');">
						<div class="icone"><img src="templates/portalTRF5/images/download.svg"></div>
						PDF
					</a>
	...
	<div class="row conteudo" data-aba-id="3">
        <div class="col-12">
            ...
            <div class="row">
                <small>Última atualização: <?php echo getDataAtualizacao2($dados['listaConvMinisterio']['return']); ?></small>
            </div>                       				
		<?php	
		foreach($dados['listaConvMinisterio']['return'] as $listDados):			
			...
			<div class="row">
				<ul class="ul-btn">
					...
					<li><a title="Link para: <?php echo $listDados['linkSistema'];?>" href="<?php echo $listDados['linkSistema'];?>" role="button" target="_blank"><div class="box aj-btn" style="color: #0275d8;"><?php echo $listDados['linkSistema'];?></div></a></li>					

...
$titleInput = "Digite um texto sobre o assunto que deseja encontrar em Serviços";
...
{
	if (strlen($title) > 240) {
		$newTitle = substr_replace($title, ' ...', 240);
		return $newTitle;
	}
	return $title;
}
...
if ($nivel == "resultado") {	?>
	...
	<div class="row">
		<div class="col-12 consultacontainer">
			<div class="box mt-2">
				Não foi possível carregar a consulta geral, <a href="#" onclick="javascript:resultadoBuscaGuiada('servicos','1','0','avancar')" title="Tente Novamente">Tente novamente</a>.
			...
			<div class="box" id="buscaGuiada">
				...
				<ul class="mt-2">
					...
					?>
								...
								<?php } else { ?>
									<li class="left">
										<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->id; ?>','<?php echo "servicos/" . $botoes->id; ?>','<?php echo $nivel + 1; ?>','','<?php echo $botoes->title; ?>', 'avancar')" ;>
											<?php echo getTitle($botoes->title); ?>
										</button>
									</li>
					<?php }
...
if ($nivel >= 1 and $nivel  <= 100) {
	$arg = $arg0;
	$subNivel = $nivel;
	$origemAberta = "portal-dos-servicos-publicos2";
	$paginaAberta = (!empty($_GET['pagina'])) ? $_GET['pagina'] : 0;
	$limiteAberta = (!empty($_GET['limite'])) ? $_GET['limite'] : 5;
	$termoBuscaAberta = (!empty($_GET['termobusca'])) ? $_GET['termobusca'] : "";
							...
							foreach ($retorno->conteudo as $botoes) {
								if (is_object($botoes)) {
									if ($botoes->exibe) {
										...
										$titulo = $botoes->title;
										if (strpos($botoes->title, "VIDEOCONFE") == true || strpos($botoes->title, "INDISPONIB") == true) {
										...
										?>
											<li class="left">
												<a class="box noborder" title="Link para <?php echo $botoes->title; ?>" href="<?php echo $link; ?>" <?php echo strpos($botoes->link, 'index.php/') ? '' : 'target="_blank"'; ?>>
													<?php echo getTitle($botoes->title); ?>
												</a>
											</li>
										<?php } else { ?>
											<li class="left">
												<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->path; ?>','<?php echo $origem . "/" . $botoes->path; ?>','<?php echo $nivel + 1; ?>','<?php echo $botoes->params; ?>','<?php echo $tituloNiveis . "/" . $titulo; ?>','avancar')" ;>
													<?php echo getTitle($botoes->title); ?>
												</button>
											</li>
					<?php }

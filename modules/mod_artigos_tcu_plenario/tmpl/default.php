...
<div class="container demonstrativo bg_azul_fundo">
	...
	<div class="row conteudo selecionado">
		<div class="col-12">
			<div class="row">
				<div class="titulo">ACÓRDÃO - TCU - PLENÁRIO - GOVERNANÇA DE GESTÃO DE PESSOAS</div>
			</div>
			<div class="row">
            ...
            //    $dado = getDados($list['governancaGestaoPessoasTcu']['anos'])[$i];
				...
				foreach($dado['meses'] as $mes): 
					...
					if($mesDesc != $mes[0]['descricaoMes']){
						...
						$liMes .= "<li><a title='Abrir ".$mesDesc."' href='#conteudo' onClick=javascript:exibirBotaoRelatorio('".$dado['ano']."-".$mes[0]['descricaoMes']."');>".$mes[0]['descricaoMes']."</a></li>";
						...
						foreach($dado['meses'] as $ano){
							foreach($ano as $listaMes){
								if($mesDesc == $listaMes['descricaoMes']){
									$anexo = $mes[0]['anexo'];
									$tabelasRow .= "<a title='Link para: ". $listaMes['anexo']['descricao'] ."' style='width: 60%;text-align: center;text-decoration: none;' class='box box-col-3' role='button' href='#conteudo' onclick=\"exibirArquivo('../gestao-orcamentaria/resultado-pdf', '".$listaMes['anexo']['id']."', 'SV21', '".urlencode("GESTÃO DE PESSOAS/ACÓRDÃO - TCU - PLENÁRIO - GOVERNANÇA DE GESTÃO DE PESSOAS/".$dado['ano']."/".$listaMes['descricaoMes']."/".retiraBarras($listaMes['anexo']['descricao']))."')\">";

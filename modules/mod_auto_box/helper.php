	  ...
	  'mes' => $mes,
		...
		try {
			...
			if($nomeServico == 'getLegislacaoInstitucional'){
				...
				$result = json_decode(json_encode($arguments), True);
				$dados['dados'] = $result['return']['legislacao']['legislacao'];
			}else if($nomeServico == 'getAtosConjuntos'){
				$responseMenu = json_decode(json_encode($client->getAtosConjuntosListaMenu()), True);
				$dados['menu'] = $responseMenu['return'];
			
				if(!empty($categoria) && !empty($ano)  && !empty($mes)  ){
					
					$responseDetalhada = array('getAtosConjuntosDetalhado' => array('categoria' => $categoria, 'mes' => $mes, 'ano' => $ano));
					$responseDetalhada = $client->__soapCall("getAtosConjuntosDetalhado", $responseDetalhada ); 
					$resultDetalhada = json_decode(json_encode($responseDetalhada), True);
					$dados['dadosDetalhado'] = (empty($resultDetalhada['return']))? "" : $resultDetalhada['return'];
					
				}					
				$arguments = array('getAtosConjuntos' => array('categoria' => $categoria));
				$arguments = $client->__soapCall("getAtosConjuntos", $arguments ); 	
				$result = json_decode(json_encode($arguments), True);
				if(!empty($result['return']['legislacao']['legislacao'])){
					$dados['dados'] = $result['return']['legislacao']['legislacao'];

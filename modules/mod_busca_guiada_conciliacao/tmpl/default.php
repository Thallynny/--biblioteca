...
$arg0 = (!empty($_GET['arg0'])) ? $_GET['arg0'] : "conciliacao";
...
$titleInput = "Digite um texto sobre o assunto que deseja encontrar em Conciliação";
...
if ($nivel == "resultado") {	?>
	...
	<div class="row">
		<div class="col-12 consultacontainer">
			<div class="box mt-2">
				Não foi possível carregar a consulta geral, <a href="#" onclick="javascript:resultadoBuscaGuiada('conciliacao','1','0','avancar')" title="Tente Novamente">Tente novamente</a>.
						...
						foreach ($retorno->categoria as $botoes) {
							if (is_object($botoes)) {
								if (!empty($botoes->link) && $botoes->link != "") {
									$link = $botoes->link; //str_replace("conciliacao/","",$botoes->link);
					?>
									<li class="left">
								...
								<?php } else { ?>
									<li class="left">
										<button title="Link para <?php echo $botoes->title; ?>" class="box noborder" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->id; ?>','<?php echo "conciliacao/" . $botoes->id; ?>','<?php echo $nivel + 1; ?>','','<?php echo $botoes->title; ?>', 'avancar')" ;>
											<?php echo getTitle($botoes->title); ?>
										</button>
									</li>
					<?php }
								...
								if (is_object($botoes)) {
									if ($botoes->exibe) {
										...
										<?php
											...
											$link = str_replace("conciliacao/", "", $botoes->link);
										?>
											<li class="left">
